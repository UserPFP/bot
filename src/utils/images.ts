...
export function validateImage(attachment: AttachmentData): ValidatedImage {
  if (
    ...
    !allowedContentTypes.includes(attachment.content_type)
  ) {
    throw new ImageError("Attachment must be an image.")
  }
  ...
  if (
    ...
    throw new ImageError(
      "The provided image must be a square (or close to it).",
    )
  }
  ...
  if (attachment.size > maxImageBytes) {
    throw new ImageError(
      "The provided image is too large and must be compressed.",
    )
  }
...
export async function validateAndDownloadImage(
  ...
  const imageArrayBuffers = await Promise.all(
    imageRequests.map(i => i.arrayBuffer()),
  )
...
export async function updateUserAvatarImage(userId: string, image: ImageData) {
  ...
  await uploadImageFile(
    `Updating animated avatar for ${userId}`,
    `${imageBasePath}.gif`,
    image.animated,
  )
  await uploadImageFile(
    `Updating still avatar for ${userId}`,
    `${imageBasePath}.png`,
    image.still,
  )
...
export async function deleteUserAvatarImage(userId: string) {
  ...
  const matchedAvatars = directoryListing.data.filter(v =>
    v.name.startsWith(`${userId}.`))
  for (const avatar of matchedAvatars) {
    ...
    if (response.data.commit.sha === undefined) {
      throw new Error(
        "Unexpected response from Github when deleting user images",
      )
    }
  }
}
